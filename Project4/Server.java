...
public class Server {
   ...
   static Group     agentGroup = new Group();
   static Vector<Integer> agentGroupIDs; 
   ...
   public static void main( String arg[] ) {
      ...
      try { svrSocket = new ServerSocket(8987); }
...
class SAgent implements Runnable {
   ...
   public void run() {
	 ...
	 try {
	    ...
	    catch ( IOException ioE ) { } catch ( NullPointerException nullPtr ) { return; }

	    if ( message == null ) { socket.close(); return ; }
	    ...
	    switch ( message.type ) {
		  ...
		  // todo
	      case Message.AGENTLOGIN : // Message(type, groupID, Vname) received.
	 		  int groupID = Integer.parseInt( message.body);
	 		  if (Server.agentGroupIDs.contains(groupID)) {
				break;
			  }
	 		  Server.agentGroupIDs.add(groupID);
	 		  Server.agentGroup.add(visitor);
	 		  break;
	 		  
	      case Message.AGENTLOGOUT: 
		  String outAVisitor_info = new String (visitor.toString());
		  ...
		  if ( group.size() < 1 && Server.defaultGroup.size() < 1 ) {
			...
			Message logoutMsg = new Message (message.type, outAVisitor_info, group.visitorList()); 
...
//		  }
  ...
  void clearAndRemoveVisitor( ) {
	   ...
	   visitor = null; in = null; out = null; 
       } catch (IOException e) { e.printStackTrace(); }
   }

   void removeGroup () {
       String rmdGroupInfo = group.toString();
       vgroup.remove( group ) ; group = null ;
