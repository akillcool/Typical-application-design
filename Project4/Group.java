...
public class Group implements Comparable<Group>, Serializable {
    ...
    public void	    add( Visitor v )	{ count ++; vecVisitor.add( v ) ; }
    public void	    remove ( Visitor v ) { count--; vecVisitor.remove( v ) ; }
    public int	    size ( ) 		{ return vecVisitor.size( ) ; }
    public void	    clear ( ) 	{ count = 0;
	if ( vecVisitor == null || vecVisitor.size() < 1 ) return;
	Visitor v;
	for ( int i = 0; i < vecVisitor.size(); i++ ) {
	    v = vecVisitor.get(i);
	    try { if ( v.out != null) v.out.close() ; } catch(IOException e) {}
	}
	vecVisitor.clear( ) ;
    }
   ...
   public static Group toGroup ( String gname_ID_size ) {
    ...
    public Group ( ) { } 
    public Group (Group grp ) { ID = grp.ID; name = new String(grp.name); count = grp.count ;} 
    public Group (int id ) { ID = id; } 
    public Group ( int id, String name ) { ID = id; this.name = new String( name ); }
    public Group ( int id, String name, int cnt ) { ID = id; this.name = new String( name );  this.count = cnt; }

    // Return a string of vname_ID; vname_ID; .... 
    public String visitorList() { return visitorList( true ) + "; " +  visitorList( false ) ; } 
    ...
    // Create and Return a group with visitors which are created based up input string. 
    ...
    public int 	   compareTo( Group g )  { return ID - g.ID; }
    public boolean equals ( Group g )  { return ID == g.ID; }
	...
	Arrays.sort( vIDArray); Collections.sort( vecVisitor );
	Iterator<Visitor>  itr = vecVisitor.iterator();
